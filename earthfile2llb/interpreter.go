...
func (i *Interpreter) handleIfExpression(ctx context.Context, expression []string, execMode bool, sl *spec.SourceLocation) (bool, error) {
	...
	for index, s := range opts.Secrets {
		...
		if err != nil {
			return false, i.wrapError(err, sl, "failed to expand IF argument %v", s)
		}
	...
	if allArtifacts {
		...
		for index, src := range srcs {
			...
			if i.local {
				...
				err = i.converter.CopyArtifact(ctx, src, dest, platform, allowPrivileged, srcBuildArgs, opts.IsDirCopy, opts.KeepTs, opts.KeepOwn, expandedChown, opts.IfExists, opts.SymlinkNoFollow)
				if err != nil {
		...
		err = i.converter.CopyClassical(ctx, srcs, dest, opts.IsDirCopy, opts.KeepTs, opts.KeepOwn, expandedChown, opts.IfExists)
		if err != nil {
...
func (i *Interpreter) handleArg(ctx context.Context, cmd spec.Command) error {
	...
	if i.local && containsShell(value) {
...
func isSafeAsyncBuildArgsKVStyle(args []string) bool {
	for _, arg := range args {
		...
		if containsShell(v) { // requires $(...) execution
			return false
		}
	}
	return true
